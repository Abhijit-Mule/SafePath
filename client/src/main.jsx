...
function App() {
  ...
  function handleUnauthorized() {
    localStorage.removeItem('safepath_token'); localStorage.removeItem('safepath_user'); setUser(null); setStats(null);
  ...
  async function loadStats() {
    const token = localStorage.getItem('safepath_token'); if (!token || user?.role !== 'authority') return;
    const response = await fetch(`${API}/reports/dashboard/stats`, { headers: { Authorization: `Bearer ${token}` } });
    if (response.status === 401) return handleUnauthorized(); if (response.ok) setStats(await response.json());
  }
  ...
  function selectImage(file) {
    ...
    setPreviewUrl(nextPreview); setForm(f => ({ ...f, image: file }));
    setMessage(file && file.size > MAX_IMAGE_BYTES ? 'Image must be 8 MB or smaller.' : '');
  }
  function locate() {
    if (!navigator.geolocation) return setMessage('Geolocation is not supported by this browser.');
    navigator.geolocation.getCurrentPosition(({ coords }) => setForm(f => ({ ...f, lat: coords.latitude.toFixed(6), lng: coords.longitude.toFixed(6) })), () => setMessage('Location permission was not granted.'));
  }
  async function submitAuth(e) {
    ...
    try {
      ...
      localStorage.setItem('safepath_token', body.token); localStorage.setItem('safepath_user', JSON.stringify(body.user)); setUser(body.user); setAuth({ name: '', email: '', password: '' }); setMessage(`Welcome, ${body.user.name}.`);
  ...
  async function submit(e) {
    ...
    if (!token) return setMessage('Please sign in before submitting a report.'); if (!form.image) return setMessage('Please choose a road image.'); if (form.image.size > MAX_IMAGE_BYTES) return setMessage('Image must be 8 MB or smaller.');
    ...
    try {
      setSubmitting(true); const response = await fetch(`${API}/reports`, { method: 'POST', headers: { Authorization: `Bearer ${token}` }, body: data }); const body = await response.json();
      if (response.status === 401) return handleUnauthorized(); if (!response.ok) return setMessage(body.message || 'Report failed.');
      ...
      if (previewUrl) URL.revokeObjectURL(previewUrl); setPreviewUrl(''); setForm({ lat: '', lng: '', address: '', description: '', image: null }); e.target.reset(); await loadReports(); await loadStats();
    } catch { setMessage('Could not reach the SafePath API.'); } finally { setSubmitting(false); }
  }
  async function changeStatus(id, status) {
    const token = localStorage.getItem('safepath_token'); if (!token) return handleUnauthorized();
    try { const response = await fetch(`${API}/reports/${id}/status`, { method: 'PATCH', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ status }) }); if (response.status === 401) return handleUnauthorized(); const body = await response.json(); if (!response.ok) return setMessage(body.message || 'Status update failed.'); setReports(items => items.map(item => item._id === id ? body : item)); setMessage(`Report marked ${status}.`); loadStats(); }
    catch { setMessage('Could not reach the SafePath API.'); }
  }
  ...
  return <main>
    ...
    {!user && <section id="account" className="panel compact"><div><p className="eyebrow">ACCOUNT</p><h2>{authMode === 'login' ? 'Sign in to SafePath' : 'Create your account'}</h2><p className="muted">Authentication is required to submit road reports.</p></div><form onSubmit={submitAuth}>{authMode === 'register' && <label>Name<input required maxLength="80" value={auth.name} onChange={e => setAuth(a => ({ ...a, name: e.target.value }))}/></label>}<label>Email<input required type="email" value={auth.email} onChange={e => setAuth(a => ({ ...a, email: e.target.value }))}/></label><label>Password<input required minLength="8" maxLength="128" type="password" value={auth.password} onChange={e => setAuth(a => ({ ...a, password: e.target.value }))}/></label><button className="button" type="submit">{authMode === 'login' ? 'Sign in' : 'Create account'}</button><button type="button" className="secondary" onClick={() => setAuthMode(authMode === 'login' ? 'register' : 'login')}>{authMode === 'login' ? 'Need an account?' : 'Already have an account?'}</button></form></section>}
    <section id="report" className="panel"><div><p className="eyebrow">NEW REPORT</p><h2>Report a road issue</h2><p className="muted">Upload a clear road image and provide its location.</p>{user && <p className="signed">Signed in as {user.name}</p>}</div><form onSubmit={submit}><label>Road image<input type="file" accept="image/jpeg,image/png,image/webp" required onChange={e => selectImage(e.target.files?.[0] || null)}/></label>{previewUrl && <div className="image-preview"><img src={previewUrl} alt="Selected road"/><span>{form.image?.name} · {(form.image?.size / 1024 / 1024).toFixed(2)} MB</span></div>}<div className="grid"><label>Latitude<input required value={form.lat} onChange={e => setForm(f => ({ ...f, lat: e.target.value }))}/></label><label>Longitude<input required value={form.lng} onChange={e => setForm(f => ({ ...f, lng: e.target.value }))}/></label></div><button type="button" className="secondary" onClick={locate}>Use my current location</button><label>Address<input value={form.address} onChange={e => setForm(f => ({ ...f, address: e.target.value }))} placeholder="Street / area" maxLength="300"/></label><label>Description<textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} placeholder="Describe the road issue" maxLength="1000"/></label><button className="button" type="submit" disabled={submitting || !user}>{submitting ? 'Submitting…' : 'Submit report'}</button>{message && <p className="notice">{message}</p>}</form></section>
