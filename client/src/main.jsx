...
function App() {
  ...
  async function submit(e) {
    ...
    try {
      ...
      setForm({ lat: '', lng: '', address: '', description: '', image: null }); e.target.reset(); loadReports(); loadStats();
    } catch { setMessage('Could not reach the SafePath API.'); }
  }
  ...
  return <main>
    ...
    <section className="reports"><div className="section-head"><div><p className="eyebrow">LIVE FEED</p><h2>Recent community reports</h2></div><span className="result-count">{filteredReports.length} shown</span></div>{loading ? <p>Loading…</p> : error ? <p className="notice">{error}</p> : filteredReports.length === 0 ? <p className="muted">No reports match this filter.</p> : <div className="report-grid">{filteredReports.slice(0, 12).map(r => <article key={r._id}><div className="card-top"><span className={`status-badge ${r.status}`}>{r.status}</span>{r.imageUrl && <a href={imageUrl(r.imageUrl)} target="_blank" rel="noreferrer">View image</a>}</div><h3>{r.location.address || `${r.location.lat}, ${r.location.lng}`}</h3><p>{r.description || 'Road condition report'}</p>{r.ai?.available && <small>AI: {r.ai.detected ? `${r.ai.detections?.length || 0} detection(s)` : 'No pothole detected'}</small>}<small>{new Date(r.createdAt).toLocaleString()}</small>{user?.role === 'authority' && <div className="status-actions">{STATUSES.map(status => <button key={status} disabled={r.status === status} onClick={() => changeStatus(r._id, status)}>{status}</button>)}</div>}</article>)}</div>}</section>
